...
export function detectAnomalies(
  ...
  for (const ids of byInvoice.values()) {
    if (ids.length > 1) {
  ...
  const anomalyIds = sortIds(
    new Set([...duplicateIds, ...roundIds, ...weekendIds])
  );
...
export const SEED_ANOMALY_DETECTION = detectAnomalies(SEED_ANOMALY_TRANSACTIONS);
