-- Seed Tier 3 continuous monitoring strategy ticket (GRC track).
--
-- Students draft a ConMon / ISCM strategy memo for a fictional system:
--   - monitoring cadence per control family
...
WHERE t.tenant_id = st.id
  ...
  AND t.scenario_brief LIKE 'ConMon:%';
...
SELECT
  ...
  'high',
  90,
  'ConMon: Draft a system-level continuous monitoring strategy for HarborNet CMS',
  jsonb_build_object(
    'systemProfile', jsonb_build_object(
      'name', 'HarborNet Case Management System (CMS)',
      'description', 'Moderate-impact web application used by a regional port authority to track vessel clearances, inspector assignments, and limited PII for credentialed contractors.',
    ...
    'prompt', 'Write a system-level ISCM strategy memo: set monitoring cadence per control family, map DefectDojo/CloudSploit/Scuba to the families they cover, and define escalation/reporting cadence for HarborNet CMS.'
  ),
  jsonb_build_object(
