...
SET
  ...
  initial_state = COALESCE(t.initial_state, '{}'::jsonb) || $igrc08${
    ...
    "breach": {
      ...
      "businessImpact": "Northwind's own checkout/ERP systems remain up; payment auth continues via the vendor after containment. Near-term impact is notification/call-center load for ~4,000 affected customers, possible PCI reassessment friction with the processor, and reputational risk from names/emails/last-4 exposure — not a Northwind outage. Quantified financial loss is not yet estimable while vendor forensics are ongoing.",
      "scopeNote": "Vendor breach (not a direct Northwind breach); exposed a subset of Northwind's customer records. Materiality is a judgment call — defend either conclusion with the factors below."
    }
  }$igrc08$::jsonb,
